...
function getNextSequenceValue(document, sequenceName) {
    var sequenceDocument = document.countDocuments()
    if (sequenceDocument == 0) {
        return sequenceName + '_' + sequenceDocument + 1
    } else {
        var sequenceValue = db.facturas.aggregate([
            {
                $addFields: {
                    sequenceNumber: {
                        $toInt: {
        ...
        ]).toArray()[0].maxSequence
        return sequenceName + '_' + (parseInt(sequenceValue) + 1)
    }
}
